...
define(['./functions', './helpers/helpers', "./environments", './tokens/tokens', './macros', './reservedWords', './options', "./logger"],
    function (functions, helpers, environments, TOKEN_TYPES, macros, RESERVED_WORDS, globalOptions, logger) {
        ...
        function parseLatex(tokens) {
            ...
            function parseMacro(macroName) {
                let macro = null;
                let isMatch = macros.isMacro(macroName)

                if (isMatch) {
                    // Check for overrides
                    macro = macros.MACROS_OVERRIDE.get(macroName);
